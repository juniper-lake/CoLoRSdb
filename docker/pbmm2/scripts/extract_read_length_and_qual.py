...
def main(args):
    """Read command line arguments and print read length and quality."""
    file = sys.stdout
    if args.readsin.endswith('.bam'):
        ...
        for b in bamin:
            ...
            print('%s\t%d\t%d' % (b.query_name, len(b.query_sequence), readqv), file=file)
        bamin.close()
        ...
        for r in fastqin:
            ...
            print('%s\t%d\t%d' % (r.name, len(r.sequence), readqv), file=file)
        fastqin.close()
    else:
