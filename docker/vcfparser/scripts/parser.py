#!/usr/bin/env python3
"""Class to facilitate parsing VCF files."""
...
class VCFParser:
    ...
    def __init__(self, infile: str):
        ...
        if file_extension == '.gz':
            ...
            raise OSError('File is not in a supported format with correct extension (.vcf or .vcf.gz)')
        ...
        if not self.next_line.startswith('#'):
            raise OSError('VCF files always have to start with a metadata line.')
    ...
    def parse_header_line(self, line: str):
        ...
        if self.header[:9] != self.header_columns:
            raise OSError('VCF header does not contain the correct fields.')
        self.samples = self.header[9:]
        if len(self.samples) < 1:
            raise OSError('This VCF file does not contain any samples.')
    ...
    def change_sample_names(self, sample_names: list):
        ...
        if len(sample_names) != len(self.samples):
            ...
            raise ValueError('Sample names can only contain letters, numbers and underscores.')
        else:
    ...
    def print_metadata(self):
        """Return a string with the metadata and header line."""
        return '\n'.join(self.metadata)
        ...
        if self.beginning:
            if self.next_line:
                ...
                if len(self.header) != len(variant_line):
                    raise SyntaxError(f'One of the variant lines is malformed: {self.next_line}')
...
class VariantRecord:
    ...
    def update_genotypes(self, ref: str, alts: list):
        ...
        genotypes = [
            map(lambda s: int(s.replace('.', str(unknown_index))), sample.split(':')[0].split('/'))
            for sample in self.sample_data
        ]

        full_genotypes = [[old_alleles[hap] for hap in genotype] for genotype in genotypes]
        new_genotypes = [
        ...
        self.sample_data = [
            ':'.join(['/'.join(map(str, new_genotypes[i]))] + self.sample_data[i].split(':')[1:])
    ...
    def check_ploidy(self, chrom: str, pos: int, regions: list[tuple[str, int, int, str, int]]):
    ...
    def pl_to_gt(self, sample_dict: dict):
        """Convert probability likelihoods to genotypes."""
    ...
    def convert_to_haploid(self, sample_idx: int):
        ...
        # deepvariant
        if 'PL' in format:
            sample_dict = self.pl_to_gt(sample_dict)
        # pbsv
        elif 'AD' in format:
    ...
    def fix_ploidy(
        ...
        if (n_sexes := len(sexes)) != (n_samples := len(self.sample_data)):
            raise ValueError(f'Number of sexes {n_sexes} does not match number of samples {n_samples}')
