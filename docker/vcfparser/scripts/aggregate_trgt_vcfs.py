#!/usr/bin/env python3
"""
This tool aggregates multiple TRGT output VCFs into a single gzipped TSV and optionally anonymizes the data by:
...
It also adds a metadata line to the vcf file to indicate that the file has been aggregated and anonymized.

TRGT VCFs should sorted and optionally gzipped.
"""
...
def parse_args(args):
    ...
    parser.add_argument("vcfs", nargs='*', help="list of optionally gzipped TRGT VCF files to be aggregated and aonymized")
...
def aggregate_trgt_vcfs(vcf_files:list, anonymize_prefix:str=None, outfile:str=None, meta_string:str='aggregate_trgt_vcfs.py'):
    """Combines multiple TRGT output VCFs into a single VCF and optionally anonymizes the data"""
    ...
    # iterate through first vcf, get corresponding records from other vcfs, aggregate, and print
    for sample1_record in vcfs[0]:
    
        # go to next record for each vcf iterator, if any of them are empty, raise IndexError
        other_sample_records = [catch(next, vcf_iterator, exception=StopIteration, handle=IndexError, message='VCFs have different numbers of variants') for vcf_iterator in vcf_iterators]
        ...
        for other_sample_record in other_sample_records:
            if sample1_record.info != other_sample_record.info:
                raise ValueError("VCFs have different INFO fields. Were all VCFs run with the same TRGT parameters and version? Are all VCFs sorted?")
            if sample1_record.format != other_sample_record.format:
                raise ValueError("VCFs have different FORMAT fields. Were all VCFs run with the same TRGT parameters and version? Are all VCFs sorted?")
            
        alts = sample1_record.alts + flatten([other_sample_record.alts for other_sample_record in other_sample_records])
        ...
        if "." in alts and len(alts) > 1:
            ...
            [other_sample_record.update_genotypes(ref=sample1_record.ref, alts=alts) for other_sample_record in other_sample_records]
            sample_data = sample1_record.sample_data + [other_sample_record.sample_data[0] for other_sample_record in other_sample_records]
            if anonymize_prefix:
                sample_data = random.sample(sample_data, len(sample_data))

            # ['CHROM','POS','ID','REF','ALT','QUAL','FILTER','INFO','FORMAT',samples]
            aggregated_record = '\t'.join([sample1_record.chrom,
                                sample1_record.pos,
                                sample1_record.id,
                                sample1_record.ref,
                                ','.join(alts),
                                ...
                                + sample_data)
            print(aggregated_record, file=file)

    # make sure the first vcf doesn't have fewer variants than the others
    for vcf_iterator in vcf_iterators:
        release(next, vcf_iterator, exception=StopIteration, handle=IndexError, message='VCFs have different numbers of variants')
...
if __name__ == '__main__':
    ...
    aggregate_trgt_vcfs(args.vcfs, args.anonymize_prefix, args.outfile, ' '.join(sys.argv))
