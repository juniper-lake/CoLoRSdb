...
def anonymize_join_vcf(vcf:str, cohort_prefix:str, outfile:str=None):
    ...
    if (n_samples := len(vcf.samples)) < 2:
       raise IOError("This VCF file only contains one sample, so anonymization is not useful.")
        ...
        vcf.change_sample_names([f'{cohort_prefix}_{i}' for i in range(1, n_samples+1)])
...
if __name__ == '__main__':
    ...
    anonymize_join_vcf(args.vcf, args.cohort_prefix, args.outfile)
