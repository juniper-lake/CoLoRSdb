...
def test_trgt_aggregation(capfd):
    ...
    out_lines = [
        "##fileformat=VCFv4.2\n",
        "##commandline=aggregate_trgt_vcfs.py\n",
    ...
    random.seed(10)
    aggregate_trgt_vcfs([vcf_file1, vcf_file2, vcf_file3], anonymize_prefix="test")
    out, err = capfd.readouterr()
...
def test_different_vcf_lengths():
    ...
    with pytest.raises(IndexError):
        aggregate_trgt_vcfs([vcf_file1, vcf_file2])
    # first vcf has more variants than at least one of the others
    with pytest.raises(IndexError):
        aggregate_trgt_vcfs([vcf_file2, vcf_file1])
