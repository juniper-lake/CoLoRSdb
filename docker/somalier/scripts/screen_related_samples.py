...
class NoRelation:
    ...
    def __init__(
        ...
        if self.pairs.columns.tolist() != self.header:
            raise OSError(f'File {infile} does not have the expected header: {self.header}')
        ...
        if self.coverages is not None:
            if len(self.coverages) != len(self.samples):
                ...
                self.samples_sorted_by_coverage = [
                    x for _, x in sorted(zip(self.coverages, self.samples), reverse=False)
                ]
        ...
        if self.pairs['relatedness'].max() <= self.max_relatedness:
            ...
            for sample in self.samples:
                relatedness_values = self.pairs.loc[
                    (self.pairs['#sample_a'] == sample) | (self.pairs['sample_b'] == sample), 'relatedness'
                ]
    ...
    def remove_related_samples(self):
        """Remove samples with relatedness above threshold."""
...
def most_common(lst):
    """Return list of most common element in a list and the count of those elements."""
    counts = Counter(lst)
...
def parse_args(args):
    ...
    parser.add_argument('--outfile', dest='outfile', help='output file name, should be TSV')
    parser.add_argument(
        '--loglevel',
        dest='loglevel',
...
def flag_related_samples(
    ...
    columns = '\n'.join(
        f'{sample}\t{keep_drop}\t{n_relations}'
        for sample, keep_drop, n_relations in zip(cohort.samples, cohort.sorted_keep_drop, cohort.n_relations)
    )
