...
import pytest
from screen_related_samples import NoRelation, flag_related_samples, most_common


def get_file(lines: list):
...
def test_most_common():
    ...
    assert most_common([1, 1, 2, 2, 3, 3]) == (
        [1, 2, 3],
        2,
    ), 'all three values occur twice'
    assert most_common(['child', 'child', 'child', 'dad', 'dad', 'mom']) == (
        ['child'],
        3,
    ), 'child would get dropped'
...
def test_sort_by_coverage():
    """Test if samples are dropped by coverage if number of relations is the same."""
    lines = [
        '#sample_a\tsample_b\trelatedness\tibs0\tibs2\thom_concordance\thets_a\thets_b\thets_ab\t'
        'shared_hets\thom_alts_a\thom_alts_b\tshared_hom_alts\tn\tx_ibs0\tx_ibs2\texpected_relatedness\n',
        'child\tdad\t0.475\t0\t982\t0.977\t293\t280\t336\t165\t480\t475\t464\t988\t0\t85\t-1.0\n',
...
def test_sample_order():
    ...
    cohort = NoRelation(file)
    assert cohort.samples == [
        'bro',
        'child',
        'dad',
        'mom',
    ], 'sample name order determined from pairs file'
    assert cohort.sorted_keep_drop == [
        'drop',
        'drop',
        'keep',
        'keep',
...
def test_flag_related_samples(capfd):
    ...
    flag_related_samples(
        ...
        sample_order=['dad', 'dad_bro', 'mom', 'bro', 'child'],
        coverages=[30, 40, 20, 50, 60],
    )
...
if __name__ == '__main__':
    ...
    # this is not executed by pytest, it's for development purposes
